...
const Board = ({boardData, mode}) => {
  ...
  const columnHints = useMemo(() => columns.map(column => generateHints(column)), []);
      ...
      [...Array(totalHeight).keys()].map(absoluteRowNum => {
        ...
        return [...Array(totalWidth).keys()].map(absoluteColNum => {
          ...
          if (playingRowNum < 0 && playingColNum < 0) {
            ...
            return <SolvableField fieldData={boardData.fields[playingRowNum][playingColNum]} mode={mode} size={size} />;
          } else {
            let hint;
            if (playingRowNum >= 0) {
              const hintId = absoluteColNum - rowHintMaxLength + rowHints[playingRowNum].length;
              hint = hintId >= 0 ? rowHints[playingRowNum][hintId] : '';
            } else {
              const hintId = absoluteRowNum - colHintMaxLength + columnHints[playingColNum].length;
